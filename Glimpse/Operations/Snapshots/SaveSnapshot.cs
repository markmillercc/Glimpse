...
[Endpoint(Method.Post, "save", Group = "snapshots")]
    ...
    public class Handler : Operation.Handler<Command, int>
    {
        ...
        protected override async Task<int> DoOperation(Command request, CancellationToken cancellationToken)
        {
            if (request.End < request.Start)
                return Unprocessable("End date cannot be before start date");
            ...
            if (request.SnapshotId.HasValue)
            {
                ...
                if (snapshot == null)
                    return Unprocessable($"Cannot find snapshot with id={request.SnapshotId} and profile id={request.ProfileId}");
