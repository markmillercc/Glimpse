    ...
    public class SaveEntry
    {
        ...
        public class CommandHandler : Operation.Handler<Command, int>
        {
            ...
            protected override async Task<int> DoOperation(Command request, CancellationToken cancellationToken)
            {
                ...
                if (subject == null)
                    return Error($"Cannot find subject with id={request.SubjectId} belonging to profile id={snapshot.ProfileId}");

                if (snapshot.Entries.Any(a => (request.EntryId == null || a.Id != request.EntryId) && a.SubjectId == subject.Id))
                    return Error($"Subject '{subject.Name}' already exists on snapshot id={snapshot.Profile}");
                ...
                if (request.CategoryId.HasValue)
                {
                    ...
                    if (category == null)
                        return Error($"Cannot find category with id={request.CategoryId} belonging to profile id={snapshot.ProfileId}");
                }
