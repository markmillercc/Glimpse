...
[Endpoint(Method.Post, "save", Group = "subjects")]
public class SaveSubject
{
    ...
    public class Handler : Operation.Handler<Command, int>
    {
        ...
        protected override async Task<int> DoOperation(Command request, CancellationToken cancellationToken)
        {
            ...
            if (profile == null)
                return Unprocessable($"Cannot find profile id={request.ProfileId}");
            ...
            if (subjects.Any(a => a.Name.ToLower() == request.Name.ToLower()))
                return Unprocessable($"Profile Id={profile.Id} already has a subject named {request.Name}");
            ...
            if (request.SubjectId.HasValue)
            {
                ...
                if (subject == null)
                    return Unprocessable($"Unable to find subject with id={request.SubjectId.Value} and profile id={profile.Id}");
